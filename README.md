...
<h4 align="center">Advanced XSS Detection Suite (formerly XSStrike)</h4>
...
<p align="center">
  <a href="https://github.com/s0md3v/XSStrike/wiki">xssniper Wiki</a> •
...
> **Note:** This project was previously called XSStrike and is now branded **xssniper**.
> The old entrypoint, `xsstrike.py`, still works exactly as before for backward
> compatibility -- but `xssniper.py` is the preferred way to run the tool going forward.

xssniper is a Cross Site Scripting detection suite equipped with four hand written parsers, an intelligent payload generator, a powerful fuzzing engine and an incredibly fast crawler.

Instead of injecting payloads and checking it works like all the other tools do, xssniper analyses the response with multiple parsers and then crafts payloads that are guaranteed to work by context analysis integrated with a fuzzing engine.
Here are some examples of the payloads generated by xssniper:
```
}]};(confirm)()//\
...
```
Apart from that, xssniper has crawling, fuzzing, parameter discovery, WAF detection capabilities as well. It also scans for DOM XSS vulnerabilities.
...
- Payload Encoding
- YAML/JSON config file overlays (see `xssniper.yaml` / `xssniper.json`)
...
Now, xssniper can be used at any time as follows:
```
python xssniper.py
```

If you have existing scripts or muscle memory from XSStrike, `xsstrike.py` is
still included and behaves identically:
```
python xsstrike.py
```
...
- [Why does xssniper boast that it is the most advanced XSS detection suite?](https://github.com/s0md3v/XSStrike/wiki/FAQ#why-xsstrike-boasts-that-it-is-the-most-advanced-xss-detection-suite)
...
- [Tool xyz works against the target, while xssniper doesn't!](https://github.com/s0md3v/XSStrike/wiki/FAQ#tool-xyz-works-against-the-target-while-xsstrike-doesnt)
